...
public class StudyActivity extends AppCompatActivity implements View.OnClickListener
        , RadioGroup.OnCheckedChangeListener, SynthesizerListener {

    //用来显示单词和音标的
    private TextView studyWordText,studyEnglishText;
    private TextView correcText,mistakeText;
    private TextView restNumText,planNumText;
    ...
    private void init(){
         ...
         */
        //用于绑定id
        studyWordText = (TextView)findViewById(R.id.study_word_text);
        studyEnglishText = (TextView)findViewById(R.id.study_english_text);

        radioGroup = (RadioGroup)findViewById(R.id.study_choose_group);
        radioOne  = (RadioButton)findViewById(R.id.study_choose_btn_one);
        radioTwo  = (RadioButton)findViewById(R.id.study_choose_btn_two);
        radioThree  = (RadioButton)findViewById(R.id.study_choose_btn_three);
        radioFour  = (RadioButton)findViewById(R.id.study_choose_btn_four);

        radioGroup.setOnCheckedChangeListener(this);
        //声明水平进度条控件
        progressBar = (ProgressBar)findViewById(R.id.progressBar1);
        correcText = (TextView)findViewById(R.id.study_correct);
        mistakeText = (TextView)findViewById(R.id.study_mistake);

        restNumText = (TextView)findViewById(R.id.study_rest_num) ;
        planNumText = (TextView)findViewById(R.id.study_plan_num);

        restNum =planNum;
        restNumText.setText(Integer.toString(restNum));//设置要测试的单词总数
        planNumText.setText(Integer.toString(planNum));//设置要测试的单词总数
        ...
        mHandler = new Handler() {
            ...
            public void handleMessage(Message msg) {
                if (msg.what == 0x111) {
                    //progressBar.setProgress(correct);    //更新进度
                    correcText.setText(Integer.toString(correct));
                    mistakeText.setText(Integer.toString(mistake));
                    restNumText.setText(Integer.toString(restNum));
                } else if(msg.what == 0x112){
                    //Toast.makeText(StudyActivity.this, "耗时操作已经完成", Toast.LENGTH_SHORT).show();
                    //progressBar.setVisibility(View.GONE);    //设置进度条不显示，并且不占用空间
                }
            }
        };
    }
    ...
    private void setTextColor(){
        ...
        radioOne.setTextColor(Color.parseColor("#383838"));
        radioTwo.setTextColor(Color.parseColor("#383838"));
        radioThree.setTextColor(Color.parseColor("#383838"));
        radioFour.setTextColor(Color.parseColor("#383838"));
        studyWordText.setTextColor(Color.parseColor("#6B8E23"));              //将单词设置为白色
        studyEnglishText.setTextColor(Color.parseColor("#616161"));           //将音标设置为白色
    ...
    private void getDBData(){
        ...
        studyWordText.setText(datas.get(k).getWord());     //设置单词
        studyEnglishText.setText(datas.get(k).getEnglish()); //设置音标
    ...
    private void setStudyContents(List<CET4Entity> datas,int k,List<Integer> list) {
        //1.设置单词内容
        studyWordText.setText(datas.get(k).getWord());
        //2.设置音标
        studyEnglishText.setText(datas.get(k).getEnglish());
        //3.设置选项
    ...
    private void btnGetText(String msg,RadioButton btn){
        ...
        if(msg.equals(datas.get(k).getChina())){
            studyWordText.setTextColor(Color.GREEN);
            studyEnglishText.setTextColor(Color.GREEN);
            btn.setTextColor(Color.GREEN);
            if(isMistake == false && alreadyMistake == false)
            isCorrect =true;
            //
            if(isCorrect == true){
                correct++;
            ...
            studyWordText.setTextColor(Color.RED);
            studyEnglishText.setTextColor(Color.RED);
            btn.setTextColor(Color.RED);
            if(alreadyMistake == false)
            isMistake = true;
            if(isMistake == true&& alreadyMistake == false){
                mistake++;
                isMistake = false;
                alreadyMistake = true;
            }
